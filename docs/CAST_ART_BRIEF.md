...
> **2026-09-24 刷新（ユーザー決裁＝案A「おすわり」）**：旧版は人間っぽさ（アニメの人間の目・頬の赤み・人の口・二足で手を組む）が
> 前面に出て「少し気持ち悪い」と言われたため、**動物の子竜**に作り直した。以後の追加・差し替えは必ずこの規則で：
> - 残す識別点：紫の鱗＋薄紫のお腹／茶色の巻き角／赤い蝶ネクタイ（赤い宝石つき）／小さなコウモリ翼／短い尾／泣き虫（目元が潤む・涙）。
> - 動物にする：丸く艶のある**動物の目**（黒い瞳＋ハイライト1つ・まつ毛/まぶたの線/眉なし）、短い爬虫類の鼻先、トカゲの口（人の唇・人の歯なし）、
>   **チーク禁止**、前足は爪つきの足（人の手・手を組むポーズ禁止）、子犬のように**おすわり**が基本姿勢。
> - 画風：**2Dアニメ塗り**（濃い紫の線画＋2〜3段のセル塗り＝ミミの立ち絵と同じ系統）。
>   ※最初に作った3D調は「プラスチックっぽくて気持ち悪い」と差し戻し（2026-09-24）＝**3D・艶・塩ビ/粘土の質感は禁止**。
>   レース中の竜（`images/dragons/poro.png`）だけは他の51頭と同じV2の3D調のまま（レース画面の統一を優先）。
> - 正本＝Higgsfield job `934b6f65-2d29-4343-9b8e-40aa868895ea`（nano_banana_pro）。表情差分・スプライトはこの job を参照して i2i。
>   （形の元＝3D調の案A `adc30bdc-…`。参照に使うと3D調に戻るので使わない）
> - 旧版は `images/cast/archive_poro_v1/`（立ち絵6・グルメレース3・散歩シート）。
...
### 正典キービジュアル（2026-08-25）
`images/kv/celestia_kv.webp`（1067x1600）＝**セレスティアの基準画**。
今後この人物を描く／生成するときは、まずこの1枚を見てから始めること。

**画の内容＝初出の台詞そのもの**：「いい眺めでしょう、ここ。島でいちばん、レース場が星に近い席。」
（`js/data_dialogue.js`）。夜の石のテラスから、はるか眼下にレース場が琥珀の灯の帯として
うねり、小さな竜たちが走っている。頭上には触れそうな天の川。足元にリュックを下ろし、
旅の手帳と星の羅針盤。黒の華奢なストラップ・ハイヒール。

この1枚に**全部**入っている：
- **格**＝左右対称の閉じた微笑・頭は水平・眉は左右同高・挑発しない
- **神眼**＝見開いた大きな瞳／深紫の中に星野と星形のきらめき／強いキャッチライト
- **ギャップ**＝世界の天井なのに、リュックを下ろした旅人の姿
- **世界**＝レースは遠く小さく、星は近い。彼女は状況の“上”にいる

光源：琥珀＝眼下のレース場とテラスの灯（前・下から）／冷たい星明かり＝左上から。
頭の後ろは暗く保つ（光輪を作らせない）。生成モデルは `seedream_v5_pro`。
ベース参照は靴＝ヒールの確定版。

### キービジュアルの罠＝頭身が退行する（2026-08-25 実測）
**同じ参照画像を渡していても、背景つきのキービジュアルでは頭身が勝手に縮む。**
立ち絵は小顔・長身なのに、シーンを描かせた途端に頭が大きく顔が丸くなり「ガキ」になった。
背景・小物・光の指示が長いほど、体型の指示が薄まるのが原因。

**対策＝頭身を最優先項目としてプロンプトの冒頭に単独で置く**：
- 「PROPORTION IS THE WHOLE ASSIGNMENT」と宣言し、9頭身・腰から踵が身長の半分以上・
  首は長く・肩は狭く、を数値で書く
- 顔は**骨格**で大人にする（面長／輪郭を細く／顎を小さく尖らせ／頬骨を高く）。
  **瞳は大きいまま**。細めるのは禁止（目力が死ぬ）
- カメラは胸の高さで水平。アオリの短縮効果で脚が縮むのを明示的に禁止
- 立ち姿はコントラポスト＋スリットで腰〜踵の脚線を通す

**検証手順（必須）**：承認済みの立ち絵とキービジュアルを、**身長を同一pxに正規化して並べる**。
頭の大きさが直接比較できる。目視で「大人っぽい気がする」は当てにならない。
実測でv1は立ち絵より明確に頭が大きく、v3で立ち絵と同等以上になった。
