...
## 1. 【最重要】歩けるマップ4枚の画風統一 — ✅ **解決済み（2026-08-01 検証）**

> **この節の前提はもう成り立っていない。再生成してはいけない。**
> 執筆（7/28）時点の粗は、その後の再生成で解消されていた。だが本書が更新されなかったため、
> **「最重要の未着手」として残り続け、次のセッションでクレジットを使って作り直すところだった。**
>
> 2026-08-01 の実測：
> - `city` / `race` / `onsen` / `beach` の4枚は **ローカルと origin/main が完全一致**（hash一致）＝デプロイ済み。
> - 4枚を等倍で切り出して横に並べて目視（本節の受け入れ手順そのもの）→
>   **筆致・パレット・輪郭の太さが揃っている**。ドット絵調もカートゥーン輪郭も残っていない。
> - さらに本節が数え漏らしていた **`fishing`（ホシウオ村）と `falls`（ルミナ瀑布）も同じ画風**。
>   ★エリアは4枚ではなく **6枚**（`js/scene_konron.js` の `KW_AREAS`）。本節の「4枚」は執筆時点の数。
> - 構図は変わっていないので **コリジョン再実測も不要**。
>
> **教訓**：指示書は書いた時点の仮説。着手前に必ず実体（画像・コード・リモート）を見る＝[[research-before-asserting]]。
> 完了した節は、その場でこの形の結論ブロックを足して閉じること。放置すると次の人の一番の時間を奪う。

<details><summary>（以下は7/28時点の記述・履歴として残す）</summary>
