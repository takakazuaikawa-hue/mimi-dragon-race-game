...
> ✅ **全9項目 消化済み（2026-07-25・指示書M0）**。内訳：
> - P0-1（称号・フロア帯が見えない）＝ハブは ui_mall_rpg.js の `rpgRenderHub2` に称号/フロア帯が実装済み。
>   さらに mall_rpg.js の `rpgRenderHub` を **delegate 化**し、旧「後勝ち上書き」依存を構造的に根絶。
> - P0-2（敵アート未配線）＝`RPG_ENEMY_IMG` を唯一の台帳に統合。canvas `rpgEnemyArt` もそこを引くため、
>   納品済み画像（en_*.webp / boss_*.webp）が戦場スプライトに出る。空の `images/rpg/enemies/` 系は廃止。
> - P1-3（HP表示/死蔵CSS）・P1-4（--scm-sub コントラスト）・P1-5（ミニマップDPR）・P1-6（タップ域44px）・
>   P2-8（ログ重複＝shop-hint bottom:64px）・P2-9（初回ガイド＝tutSeen）＝いずれも先行セッションで対応済み。
> - P2-7（reduced-motion 網羅漏れ）＝rpgpulse 全13クラスを一括停止するブロックを追加して完済。
> 以降にモールの新しい課題が出たら、この下に項目を足す。
