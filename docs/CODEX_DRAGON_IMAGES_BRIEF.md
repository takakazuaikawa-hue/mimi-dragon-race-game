...
- 背景：**★透過（アルファ付きPNG）を第一希望**——この画像は図鑑だけでなく**レース画面へのHD2D合成**にも使うため。
  生成モデルが透過を出せない場合のみ「無地の純白」で納品（こちらで抜く）。**どちらか一方に40枚全部を統一**すること（混在禁止）。
  地面の影・小物・フレームは入れない（合成時に邪魔になる）。
- 文字・ロゴ・透かし：**一切入れない**
- 1枚に**その竜1頭のみ・全身**・**頭が右向き**（既存12枚と同じ向き＝レースの進行方向）
- ポーズ：既存踏襲＝性格の出る立ち姿/浮遊姿でよい（**走りポーズの強制はなし**＝走行の動きはゲーム側で付ける）。
  cloud_chaser系・wing_closer系・tier7神格は浮遊/滑空ポーズ可（momu.png が浮遊ポーズの見本）。
- ファイルサイズ目安：1枚 150KB以下（PNGが重い場合はWebP圧縮でも可＝既存12枚も実体はWebP。拡張子は .png のまま。**透過を潰さないこと**）
...
## 🔍 1枚ごとの合格チェック（保存前に必ず）
1. **背景透過（第一希望）or 純白（40枚統一）**・単体・全身・**頭が右向き**・文字/透かしなし・地面影なし
