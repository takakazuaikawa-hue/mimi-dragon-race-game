...
### 11.6 実機フィードバック後の改修（2026-09-25「全体的に動きが固い」）
- 羽ばたき＝翼根ピボットの**回転**＋打ち下ろしで**縦に縮める**（手前へ振り下ろす遠近）＋先端の遅れ。打ち下ろし速く・振り上げ遅くの非対称波（`_rcWingStroke`）。
  以前は縦帯を上下にずらすだけ（せん断）で、打ち下ろしが無く、46px で先端が約4px しか動かなかった。
- 胴＝打ち下ろしで浮く（`_rcWingLift`）＋機首が僅かに上がる／尾＝翼より 1.3rad 遅れてしなる（振れ幅 0.11→0.16）。φ=0 は元の絵のまま（図鑑・カードの静止画は不変）。
- しなりの帯数を画面上の幅に比例（約3px/帯・8〜64本）。固定10本だとウィニングカットで翼が階段状にギザギザだった。
- `tools/dragon_v2_rig.py`：翼マスクの縁を原寸の差分から取り（NEAREST 拡大の 4px 階段を解消）、胴と同色の画素を翼から外し、アルファだけに掛ける（暗い縁取り・付け根の割れ目を解消）。
  `python3 tools/dragon_v2_rig.py rebuild` で 52頭を各 meta.json の設定のまま作り直した（`RC_DRAGON_ART_V` 2→3）。

### 11.7 レース画面の残課題つぶし（2026-09-25「課題と呼べるものはすべて解決」・すべて表示専用）
- **竜と文字の重なり**：番号＋名前の札を竜ごとに即描きせず、全頭を描いた後に `rcDrawRaceLabels` で**画面座標**に配置する。
  候補＝体のすぐ下→すぐ上→一段下→一段上（左右ずらし付き）。1巡目は他の札・竜の体・スタート/ゴール門の看板を避け、2巡目は体の上だけ許す。
  入らなければ番号の丸だけに縮め、賭けた竜の札は必ず出す。点呼プレートは画面内にクランプ、浮き文字（`addFloat`）とポップは重なったら上へ積む。
- **道と縁石が2本**：L2（中景）は自前の路面と縁石を下部に持つため、下端 80%→**97%** に下げて L3 の地面の下に隠す（`bakePara` の spec）。
  lowSpec は L2 を省くので、代わりに L1 へ L2 を焼き込んだ `paraBaked.L12` を使う（L1 の縁石が露出しない）。
- **L3 の継ぎ目の二重透け**：納品の L3 は左端 0〜約110px が「右端の続き→左端の絵」のクロスフェードで、高さの違う縁石が二重に透けていた。
  `python3 tools/racebg_loop_l3.py`（納品時の 1600px 幅のみ処理）で、①継ぎ足し 113px を落とす ②地域ごとの縁石の直線（`KERB` 表・5時間帯共通）を
  上端・下端固定の縦伸縮で水平にそろえる ③納品のクロスフェード部を捨て、縁石の帯の絵柄が一致する列どうしを 120px でつなぐ。描画側は L3 をそのまま並べる。
  背景を差し替えたら `KERB` 表を読み直すこと（霧・岩で片端に縁石が無い地域は None＝伸縮なし）。
- **スタート/ゴール門の画風**：細い単色ポール＋素のチェッカー＋文字がはみ出す小札（ドット絵のUIに見えた）→ `rcDrawGate`：
  台座・柱頭つきの円柱陰影の柱、布の丸みと接地の暗がりを乗せたチェッカー（純白×純黒にしない）、文字幅に合わせた金縁の看板。
  時間帯の明るさ `gateLt`（昼1.0〜夜0.62）で全体を沈める。スタート門の柱は格で 木→石→金。
