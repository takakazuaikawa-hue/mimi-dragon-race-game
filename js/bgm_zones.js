...
(function () {
  // ★全13ゾーンの結線は完了済み（docs/BGM_ORDER_BRIEF.md／[[audio-volume-control]]）。
  //   ゾーン＝「部屋」単位。曲は bgm/uibgm/ に置かれた実ファイルだけを使い、**未納品のゾーンは無音**
  //   （probeZone で実在チェック→無ければ stop＝置く前に結線しても壊れない・1曲届くたびに自然に鳴りだす）。
  //   ★納品状況（2026-09-19 実測）＝**届いているのは T1 title-konron.mp3 と T2 home-morning.mp3 の2本だけ**。
  //     残り12ゾーンは mp3 が存在しないので無音のまま（＝設計どおりで、壊れてはいない）。
  //     曲が届いたら BGM_ORDER_BRIEF §4 のパスへ置くだけでそのゾーンが鳴りだす＝このファイルは触らなくてよい。
  var HOME_TRACKS = [
