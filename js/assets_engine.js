...
//   3話で資産形成に入る方が物語に合う」＋第2便「3万は10億を持つ人間の借金として軽い」）。
//   **完全に表示上の物語**＝内部の資産計算・関門・レース数値には一切影響しない。
//   構図：総額300万（船賃・宿代・装備の立替）。うち297万は**サケが立て替えた**（竜王女の顔）。
//   ミミの自力返済分＝3万＝第3話の関門（総資産3万）と同額。つまり
//   「到達最高資産が3万になる」ことを序盤は「自力分を返し終える」として見せる。
//   サケ立替分は資産300万到達時の恩返しイベント（sake_repay）で物語として決着（コインは動かさない）。
const MIMI_DEBT_TOTAL = 3000000;   // 総額300万（物語上の重さ）
const MIMI_DEBT0 = 30000;          // 自力返済分＝第3話関門と同額（完済＝資産開示）
const MIMI_DEBT_COVERED = MIMI_DEBT_TOTAL - MIMI_DEBT0;   // サケ立替 297万
