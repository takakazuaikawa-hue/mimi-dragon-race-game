  ...
  wrap.innerHTML = `
    ...
    <div class="title-inner">
      ...
      <!-- ★サブタイトルは画像に焼かずHTMLの文字にする。旧ロゴは焼き込みだったため
           ...
           ★置き場所はロゴ直下ではなく画面の下寄り（2026-08-04・実機指摘）。
           ロゴ直下だと空・遠景の城・飛ぶ竜という「絵の主要な部分」を塞いでいた。
           ミミの腿のあたりなら、隠すのは衣装の布だけで済む。 -->
...
function renderSettings() {
  ...
  app.appendChild(el("div", "set-ver", "ミミのドラゴンレース紀行"));
...
function renderHelp() {
  ...
  // ★同着＝賭けが成立しない、という帳場のルール。終章の最終決戦で効いてくる伏線でもあるが、
  //   ここでは戦術ではなく**ただの帳場の決まりごと**として置く（答えを教えない）。
  //   詰まった人が最後に引きに来る場所なので、辞書としてここに無いと逃げ場がなくなる。
  app.appendChild(section("deadheat", "同着になったら", [
    "鼻先が完全に並び、写真判定でも分けられないとき——その決着は <b>同着</b> として記録されます。",
    "聖龍レースでは、当たりが同着になったとき、配当は <b>1.0倍</b>。賭けたぶんが、そのまま返ってきます。",
    "同着は竜の <b>名誉にも、勝敗にもならない</b>——という古い言い伝えから続く、この島の決まりごとです。勝った竜も、負けた竜も、いないのですから。"
  ]));
...
function _tcgCardEl(d, no, entry, big, lore) {
  ...
  const rar = dexRarity(d), num = "No." + String(no).padStart(2, "0");
  const card = el("div", "tcg-card " + (seen ? "rar-" + rar.key : "back") + (big ? " big" : "") + (fav ? " fav" : ""));
  card.style.setProperty("--dc", (typeof dragonColor === "function") ? dragonColor(d) : (d.color || "#888"));
  if (!seen) {
    card.innerHTML = `<div class="tcg-back"><div class="tcg-back-emb">竜</div><div class="tcg-back-q">？？？</div><div class="tcg-back-no">${num}</div></div>`;
    return card;
  }
  ...
  card.innerHTML =
    `<div class="tcg-face">` +
      ...
      `<div class="tcg-art"><span class="tcg-rar">${"★".repeat(rar.stars)}</span><canvas width="${aw}" height="${ah}" data-w="${aw}" data-h="${ah}"></canvas>${fav ? `<span class="tcg-fav">★</span>` : ""}</div>` +
...
function showCollectionDragonDetail(d) {   // ※poro.js の showDragonDetail(id) と名前衝突していたため改名（図鑑=オブジェクト渡し）。
  ...
  const big = _tcgCardEl(d, DRAGONS.indexOf(d) + 1, entry, true, notes[0]);
...
function renderCollection() {
  ...
  // 竜カード・グリッド＝トレーディングカード風（レア度の枠・ホロ・未発見はカード裏）。表示のみ。
  const grid = el("div", "dex-grid tcg");
  let shown = 0;
  DRAGONS.forEach((d, idx) => {
    ...
    const card = _tcgCardEl(d, idx + 1, entry, false);
...
async function shareGameInfo() {
  ...
  const payload = { title: "ミミのドラゴンレース紀行", text };
