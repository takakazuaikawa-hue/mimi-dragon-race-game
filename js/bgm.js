...
const TRACK_GAIN = {
  ...
  // ★bgm/uibgm/（2026-09-19 実測：**在庫は title-konron.mp3 の1本だけ**。以下のゲインは
  //   納品前に先行登録されたもので、対応するmp3はまだ存在しない＝この表は当たっても外れてもいない。
  //   曲が届いたら 0.1195÷実測RMS で**測り直す**こと（先行値のまま使うと音量が揃わない）。
  //   ※「Suno納品13曲」と書かれていたが、プロファイル全体を走査しても11曲は存在しなかった。
  //   ※★RMS の近さで曲を同定しようとしないこと：home-morning の実測 0.1671 は、別曲である
  //     documentary の登録値 0.1673 とほぼ一致する。Suno 出力は音量が近い値に集まるだけ。）
