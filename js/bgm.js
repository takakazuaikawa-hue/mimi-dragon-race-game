...
const TRACK_GAIN = {
  ...
  // ★bgm/uibgm/（2026-09-19 復旧：**14曲＋当たり音3本すべて在庫あり**）
  //   一度この階層が空になっていたが、退避コミット 65362cf（2026-08-25・codexブランチ上）に
  //   全ファイルが残っていた。mainを早送りした際に取り残されたのが原因＝曲は失われていなかった。
  //   以下のゲインは復旧した実ファイルに対して全サンプルで再測定し、**誤差0.0〜0.1%で一致**を確認済み。
  //   ※★RMSの近さで曲を同定しないこと：別曲の home-morning(0.1671) と documentary(0.1673) は
  //     ほぼ一致する。Suno出力は音量が近い値に集まるだけで、一致は同定の根拠にならない。
