...
function rareChecks(seq) {
  ...
  //   ★ただし発火は「その解説者が担当 × 条件成立 × 抽選」の積なので、
  //     滅多に起きない条件（鼻先決着×特定の解説者）は数百レースでは
  //     たまたま出会わないことがある。実測でウンメの鼻先決め台詞は
  //     600レースで5回・800レースで全20種発火。300で切ると誤検出になる。
  //     そこで「本当に出せないのか」を判定するのは800レース以上のときだけ。
  const never = R.concat(C).filter(r => !count[r.id]).map(r => r.id);
  if (never.length && seq.length >= 800)
...
function chatterChecks(seq) {
  ...
  const VOICE = {
    // ★一人称はユーザー指定：サケ=わし／ミズ=あたくし／スミカ=私／
    //   マクラ=アタシ／セレスティア=私／ウンメ=ウンメちゃん
    //   （スミカとセレスティアが同じ「私」なので、一人称だけでは分けられない。
    //     語尾で見分ける：スミカ=丁寧語、セレスティア=のね／のよ／こと）
    sake:     { mark: /じゃ|のう|わし|とる|わい|でな|んじゃ/,
                ng: /あたくし|アタシ|ウンメちゃん|ございます|ですよぉ/ },
    mizu:     { mark: /わ。|のよ|わね|かしら|あたくし|あはん|なさい|だけよ|のね/,
                ng: /わし|アタシ|ウンメちゃん|ございます|じゃよ/ },
    sumika:   { mark: /ござい|ます|です|いたし|私|くださいませ/,
                ng: /わし|アタシ|あたくし|ウンメちゃん|知らんけど|じゃん/ },
    makura:   { mark: /じゃん|って|アタシ|マジ|ヤバ|だよ|くない|んだ|すぎ/,
                ng: /わし|あたくし|ウンメちゃん|ございます|のじゃ/ },
    celestia: { mark: /のね|のよ|私|ふふ|こと。|なのよ/,
                ng: /わし|アタシ|あたくし|ウンメちゃん|ございます/ },
    unme:     { mark: /です|ますぅ|ウンメちゃん|知らんけど|ましたぁ|ですって|ねぇ|よぉ/,
                ng: /わし|アタシ|あたくし|ございます|じゃよ/ }
  };
